"""Standalone TrackFormer Trackformer1.0 architecture — the best model in this project (434.96 km RMS track
error, 10-seed ensemble, WP+EP 2020+ full-20-lead test set). Chain-of-thought (CoT) steering-flow
prediction (v21) plus a temporal history of that steering representation (Trackformer1.0's addition).
...
class HistStem(nn.Module):
    """v17's steering stem, plus a zero-initialised residual carrying t-12h and t-24h (Trackformer1.0)."""
...
class TrackFormerHist(TrackFormerCoT):
    """Trackformer1.0: v21 + a temporal history of the steering representation (t-12h, t-24h). This is the
    class the released Trackformer1.0 checkpoints instantiate."""
...
_stats = _np.load(_os.path.join(_os.path.dirname(__file__), "trackformer10_norm_stats.npz"))
