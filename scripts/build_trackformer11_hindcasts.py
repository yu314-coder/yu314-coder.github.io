#!/usr/bin/env python3
"""Publish Trackformer1.1 runs for the track-history explorer.

Track-history mode cannot call Trackformer1.1 the way live mode does. Trackformer1.1 integrates a
...
every slider move, client-side and instantly, while one Trackformer1.1 initialisation needs
~34 MB of GRIB decoded first.

So published Trackformer1.1 runs are turned into a static file instead. Point this at
...
Because the structure head landed, Trackformer1.1 no longer needs Trackformer1.0 for anything here.
The cone is now Trackformer1.1's own member spread rather than borrowed error statistics.

Re-run this whenever a new Trackformer1.1 case is published and that storm lights up in the
...
OUT = REPO_ROOT / "assets" / "typhoon-tracker" / "model" / "trackformer11-hindcasts.json"
...
def cone_from_members(fc, members):
    ...
    the deterministic route. Trackformer1.1's own spread -- nothing borrowed."""
    if not members:
        return None
    out = []
...
def main():
    ...
    for path in MANIFESTS:
        ...
        entry = {
            ...
            "model": case.get("model") or "Trackformer1.1 causal route + intensity/structure head",
    ...
    payload = {
        ...
        "note": ("Published Trackformer1.1 runs only. History mode uses Trackformer1.1 where an entry matches the storm and "
                 "initialisation on screen, and Trackformer1.0 everywhere else -- Trackformer1.1 needs real analysis fields, "
                 "which do not exist for most past storms."),
        "cone": f"{CONE_PERCENTILE:.0f}th-percentile radius over Trackformer1.1's own {SPAGHETTI_MEMBERS}+ route members",
        "hindcasts": out,
        "skipped": skipped,
    }
